...
signalsSymbols = []
topInputs = []
topInputs,signalsSymbols,signalsTable,elementsTable=initializer("demofile.txt",topInputs,signalsSymbols)
...
# FROM THIS POINT ON WE WILL SORT
def sort(signalsSymbols,topInputs,signalsTable2,elementsTableAnd):
    # we run the circuit
    order = []
    indexOrder=[]
    orderElem=[]
    for i in range(len(elementsTableAnd)):
        process(elementsTableAnd[i], signalsTable2)
        #print(signalsTable2)
        for j in range(len(signalsTable2)):
            if(signalsTable2[j] != 2):
                updatedSignal = j
                indexOrder.append(j)
                exist = order.count(chr(updatedSignal+97))
                #print("found a non 2:",signalsTable2[j],"and we are checking for this char: ",chr(updatedSignal+97))
                if(exist == 0):
                    #print("I entered here")
                    order.append(chr(updatedSignal+97))
    #----trying
    # print(elementsTable)
    # for i in range(len(indexOrder)):
    #     # get the first layer of the circuit in first
    #     #print(signalsSymbols[indexOrder[i]])
    #     for j in range(len(elementsTable)):
    #         if(signalsSymbols[elementsTable[j].output]==signalsSymbols[indexOrder[i]]):
    #             print("got in")
    #             orderElem.append(elementsTable[j]) 
    #     #orderElem.append(elementsTable[ord(topInputs[i])-97])
    
    return order


# time to initialize our stuff for sorting
# signalsTable is all 2, if an index is not 2, we add it -if it doesnt already exist- in a sorted list
signalsTable2 = []
for i in range(len(signalsTable)):
    signalsTable2.append(2)

allAndElements = []
# now we will create an all AND elements table so it "lights up" if both inputs are there.
for i in range(len(elementsTable)):
        # we initialize as many elements as the lines of the file as AND gates, but later we will change them
        if(elementsTable[i].gate == 'NOT'):
            E1 = element('NOT',elementsTable[i].input1,elementsTable[i].input2,elementsTable[i].output)
            allAndElements.append(E1)
            continue
        E1 = element('AND',elementsTable[i].input1,elementsTable[i].input2,elementsTable[i].output)
        allAndElements.append(E1)
print(allAndElements)
print(signalsTable2)
for i in range(len(topInputs)):
    signalsTable2[ord(topInputs[i])-97] = 1
print(signalsTable2)
order = sort(signalsSymbols,topInputs,signalsTable2,allAndElements)
print("Order: ",order)